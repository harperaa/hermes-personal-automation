...
**The dashboard door — Cron → Blueprints.** On the Hermes Plugins image the same eleven appear as a numbered *Personal Automation* section above Hermes' built-in gallery, each one a form: pick when it runs (every weekday, daily, weekly on chosen days, monthly, every N hours, or raw cron — the plain-English result and the server time zone are shown), where it delivers, what it should focus on (that text is the instruction the job runs the skill with, so "also flag anything from Costco" is a one-line edit), and the skill's own settings, which are written to `skills.config` exactly as `hermes config set` would. File settings open an inline editor seeded with the example from this workshop (a missing file is created on Save, folders show their files as chips with the starter files we ship), so nobody has to find `~/life/` by hand. Schedule creates the same `blueprint:<name>` job `/suggestions accept` creates and marks the suggestion accepted; Remove deletes the job and puts the suggestion back. Either door, same result.

**Updates reach you on the image.** On the Hermes Plugins image the boot seed keeps a manifest of what it shipped (`~/.hermes/personal-automation-seed.json`). When a newer version of a skill ships and your copy is still exactly what we installed, it is replaced on the next boot; if you edited the SKILL.md, yours stays and the boot log says so (delete the folder to take ours); a copy installed before the manifest existed is backed up to `~/.hermes/skills-backup/` and replaced. Your jobs, suggestions and input files are never touched by this.

...
**The dashboard door — Cron → Blueprints.** On the Hermes Plugins image the same eleven appear as a numbered *Personal Automation* section above Hermes' built-in gallery, each one a form: pick when it runs (every weekday, daily, weekly on chosen days, monthly, every N hours, or raw cron — the plain-English result and the server time zone are shown), where it delivers, what it should focus on (that text is the instruction the job runs the skill with, so "also flag anything from Costco" is a one-line edit), and the skill's own settings, which are written to `skills.config` exactly as `hermes config set` would. Schedule creates the same `blueprint:<name>` job `/suggestions accept` creates and marks the suggestion accepted; Remove deletes the job and puts the suggestion back. Either door, same result.
